            ...
            return {
                ...
                link: function (scope, element, attrs) {
                    ...
                    if (isLabel) {
                        ...
                        var watchRecord = scope.$watch(workString + 'record.' + attrib, function (newVal) {
                            if (newVal) {
                    ...
                    if (isLabel) {
                        var watchRecord_1 = scope.$watch('$parent.record.' + attrs['fld'], function (newVal) {
                            if (newVal) {
                                if (/^[a-f0-9]{24}$/.test(newVal.toString())) {
                                }
                                else if (scope.$parent["f_" + attrs['fld'] + "Options"]) {
...
(function (fng) {
    ...
    (function (services) {
         ...
         */
        /*@ngInject*/
        recordHandler.$inject = ["$http", "$location", "$window", "$filter", "$timeout", "routingService", "cssFrameworkService", "SubmissionsService", "SchemasService"];
        function recordHandler($http, $location, $window, $filter, $timeout, routingService, cssFrameworkService, SubmissionsService, SchemasService) {
                    ...
                    var _loop_1 = function () {
                        ...
                        if (schemaI.schema) {
                            ...
                            if (idList_1 && idList_1.length > 0) {
                                ...
                                var thisConversion_1 = getConversionObject($scope, fieldname, schemaName);
                                if (thisConversion_1) {
                                    ...
                                    if (schemaI.array) {
                                        ...
                                        if (lookup) {
                                            for (var n = 0; n < lookup.length; n++) {
                                                newVal[n] = convertLookup(lookup[n], thisConversion_1);
                                            }
                                        }
                                    }
                                    else {
                                        newVal = convertLookup(lookup, thisConversion_1);
                                    }
